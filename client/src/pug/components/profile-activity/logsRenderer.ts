...
export const logsRenderer = (filter: string): string => {
    ...
    const {
 bookings, logins, logouts, signup 
} = getUserFromStorage().logs;
...
function generateActivityRecord(date: number, type: string): string {
    ...
    const month: string = (recordDate.getMonth() + 1) < 10 ? `0${(recordDate.getMonth() + 1)}` : `${(recordDate.getMonth() + 1)}`;
